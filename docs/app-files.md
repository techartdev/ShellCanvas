...
Production SSH and installed adapter listings now reach this iterator through demand-driven native readers. The broker buffers one provider page and does not read ahead. Legacy providers and preview services without a reader still use a materialized compatibility snapshot. Bundled Files and system pickers consume the same reader contract progressively, retain discovered entries for their view, and bound the rendered rows to the viewport. See [native directory readers](native-directory-readers.md) for lifecycle and cancellation rules; recursive folder transfers use their separate incremental engine.
