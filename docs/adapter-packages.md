...
The installer rejects links, escaping paths, case aliases and reserved Windows names. Metadata has a 1 MiB parsing bound. A package may contain at most 4096 declared files and 512 MiB of payload. Asset copying and hashing use 64 KiB chunks. Review captures a staged snapshot, so changing the original folder cannot change the package being approved. Review requests are window-owned, cancelable, single-use and expire after ten minutes.
...
The latest Windows run passes 75 checks. It includes a third package generated,
...
hit its 25-second watchdog; an unchanged rerun completed all 71 checks. The current
expanded fixture uses a 45-second native watchdog and a 60-second runner deadline.

Three directory checks exercise the installed SDK app through the native reader:
one-page demand followed by early close, all 350 entries in three pages, and
cancellation while paused after the first page. The first attempt exposed missing
reader forwarding in the app permission scope; correcting that routing made all
74 checks pass. These checks count native reader calls and await cleanup rather
than accepting pages from a materialized compatibility listing.

The bundled-browsing check additionally confirms that Files consumes all three
native pages, closes its reader, shows the complete 350-entry count and renders
fewer than 80 rows. The progressive UI and large-directory browser fixture is
described in [native directory readers](native-directory-readers.md).
