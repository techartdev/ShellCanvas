...
- The control channel allows 64 outstanding calls in each direction, messages up to 4 Mi UTF-16 units and JSON values up to 64 levels deep. These are control-message resource budgets, unrelated to remote folder size/depth. State events use one cancelable read with bounded history and explicit snapshot reset. Binary streams, their backpressure and native-process framing are not implemented yet.
