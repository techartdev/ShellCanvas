...
Production setup still offers SSH only. Per-binding availability events and UI, explicit service IDs/generations, independent-leg reconnect/rebinding, composition configuration and a mixed desktop UI fixture remain before BASE-09/10 are complete. The existing whole-workspace reconnect flow remains unchanged. General API/command services and adapter configuration schemas remain separate contracts.
