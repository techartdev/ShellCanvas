...
Production setup still offers SSH only. A subsequent [service availability](service-availability.md) slice adds native snapshots, polling, partial UI and a mixed desktop fixture. Push events, explicit service IDs/generations, independent-leg reconnect/rebinding and composition configuration remain before BASE-09/10 are complete. The existing whole-workspace reconnect flow remains unchanged. General API/command services and adapter configuration schemas remain separate contracts.
