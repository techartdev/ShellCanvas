...
App-owned [local data and settings](app-storage.md) now have an explicit permission, paginated keys, revision-checked writes/deletes and cancellation. Data persists independently of packages and sessions. The SDK and Field Notes example use the brokered API; clipboard, events/discovery and the remaining lifecycle services are still required for the app-services gate.
