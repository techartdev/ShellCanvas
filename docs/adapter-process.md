...
One process represents one established connection generation. Clones and service handles retain that generation. `close()` invalidates dispatch immediately, cancels pending work and supervises process cleanup; all close waiters observe the same cleanup result. Dropping the last owner also starts cleanup. Cleanup continues if its caller is canceled. A separate adapter process remains usable when another exits or violates the protocol.

On Windows the host creates an unnamed, non-inheritable [job object](https://learn.microsoft.com/en-us/windows/win32/procthread/job-objects) with `JOB_OBJECT_LIMIT_KILL_ON_JOB_CLOSE` and no breakaway flags. The adapter starts with `CREATE_NO_WINDOW | CREATE_SUSPENDED`, joins that job, then resumes. Stable Rust does not expose its primary thread handle; the host enumerates the fresh suspended process, checks ownership and requires an unambiguous startup thread. Failed assignment or unexpected startup state stops the child and refuses the connection. No adapter code is intentionally run before job assignment.

Ordinary child/grandchild processes inherit the job. Cleanup captures identity-pinned handles for current members, terminates the job, reaps the root and waits for zero active job members and captured process exit signals under a two-second deadline. PID reuse cannot redirect this wait to an unrelated process. A changing process list can grow the native snapshot; repeated churn or OS failures produce unconfirmed cleanup rather than a false success. If cleanup is unconfirmed or the supervisor task is abandoned, its installed-package owner remains retained. Closing the job handle is a termination fallback, including when the host process crashes; Tokio's `kill_on_drop` remains a direct-child fallback.

This is lifecycle supervision for trusted native code, not a sandbox. Programs retain the user's permissions, and processes created indirectly through external system brokers/services are not generally descendants in the job. Adapters must clean up their remote resources and avoid starting unmanaged background services. Non-Windows hosts currently retain direct-child cleanup; process-group implementation and platform evidence remain required. See [Microsoft's creation flags](https://learn.microsoft.com/en-us/windows/win32/procthread/process-creation-flags) and [Tokio process ownership](https://docs.rs/tokio/latest/tokio/process/struct.Command.html#method.kill_on_drop).

Three Windows integration tests exercise real adapter/child/grandchild processes: close/drop, adapter crash/protocol failure with a surviving independent tree, canceled/rejected initialization after helpers start, and termination of the process owning the adapter job. Tests pin process handles and require them to signal exit; PID absence alone is not evidence. Run `cargo test -p shellcanvas-adapter-runtime --test process_tree --locked`. The installed-adapter desktop fixture also passes all 68 checks with this supervisor, including generated package installation, composition, source replacement, console I/O and file transfers. These fixtures use no remote devices or user clipboard.
...
Paths, parent locations, roots and cursor tokens remain opaque. The host does not split/join/normalize them or send them to a different service source. The [native directory reader](native-directory-readers.md) fetches a page per `next()` and retires canceled/failed scans. Native IPC and the public app broker preserve this demand-driven behavior. Bundled Files and system pickers consume those readers progressively and close them when navigating away. The explicit compatibility `list()` bridge still collects pages, with a 30-second operation deadline. Recursive transfers use the separate incremental directory reader described below. There is no new total-entry count limit.
