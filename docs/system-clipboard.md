...
Links, junctions, reparse points and special files are rejected rather than followed. Windows destination names are validated for traversal, reserved names and case collisions; Explorer virtual paths must be shorter than 260 UTF-16 code units, the size of its descriptor name field including the terminator. Download does not have that clipboard-specific limit (normal filesystem limits still apply). Windows requires a contiguous [FILEGROUPDESCRIPTORW metadata array](https://learn.microsoft.com/en-us/windows/win32/api/shlobj_core/ns-shlobj_core-filegroupdescriptorw) when Explorer requests the selection. ShellCanvas fills that block directly from the catalog, then serves indexed file streams on demand. This native format still requires memory proportional to the descriptor count, so an Explorer offer is limited to 250,000 descriptors (about 141 MiB); discovery stops as soon as a selection exceeds it and suggests Download. That bound belongs to this clipboard format only. Download, Upload, remote Copy and Paste into the transfer queue have no total entry or metadata limit: the disk-backed catalog is bounded by free space on the temporary-files volume, and a full disk fails the transfer with that explanation. Separately, every catalog entry (selection roots, provider entries and local files alike) is refused when its decoded path, name, kind and revision strings together exceed 4 MiB of UTF-8. That counts those four strings only, not the stored row. Built-in SFTP name and path validation keeps entries far below it, and a test-only assertion keeps the adapter frame maximum within it without a production dependency on adapter framing. Explorer descriptor paths are derived for presentation only and stay bounded at any depth; an offer containing one of 260 UTF-16 code units or more is refused before publication, never shortened. Transfers, conflict checks and errors always use the full provider path and name.
...
- Opt-in `live_folder_roundtrip` exercised the production native tree engine against an authorized Linux test host: local folder upload, remote folder copy, downloads, exact byte comparison, Unicode names, a zero-byte file and an empty nested folder. The disposable UUID remote root was removed.
...
- The live authorized-host folder round trip passed again through the new engine, including local upload, remote copy, exact download bytes, Unicode names and empty directories. The owned remote fixture was removed.
...
### Total-tree budgets removed again — 2026-09-11

- The 0.1.0 desktop build refused discovery beyond 250,000 catalog entries or 128 MiB of catalog metadata (release-preparation commit `27afacb`, with no associated validated finding). Those totals limited legitimate operations rather than untrusted inputs or concurrent work, and contradicted the transfer contract above. Queue transfers are unbounded again. Page size, one active directory cursor, the SQLite page cache and job/worker counts stay bounded. One new per-entry rejection was added: 4 MiB across an entry's decoded path, name, kind and revision (not a row size). Stored Explorer display paths no longer grow with tree depth; they remain presentation only and overlong ones still refuse the offer.
- A synthetic provider supplied 250,127 files through one cursor in pages of at most 128; no file contents were opened and the scratch catalog was removed afterwards. Canceling after about 20,000 discovered entries reported `canceled`, released the cursor and removed the catalog. A simulated full disk (SQLite page limit) reported storage exhaustion as `failed`, never as canceled or as a folder conflict, with the same cleanup. An in-memory consumer limit stops discovery after the first page beyond it.
- Opt-in `discovery_exceeds_the_former_metadata_budget` wrote a catalog larger than 128 MiB from 5,000 entries with 32 KiB opaque paths. It writes about 0.5 GB; run it with `TMP` on a volume with space and `--ignored`.
- Local gate over the integrated tree, with builds and scratch data on a secondary volume: 291 frontend tests (52 files), 232 Rust tests (4 opt-in/live ignored), SDK checks, formatting, production build and all-target Clippy passed. Synthetic providers only; no live host or Explorer walkthrough was repeated.
- Follow-up coverage: two entries with distinct long names whose shortened displays collide are refused by `explorer_sources`, the only production path from a catalog to Explorer descriptors, while their full provider paths and names stay intact. A real partial chooser download canceled after its first file is reported as `canceled`, with the completed file preserved and named in the message. Discovery cancellation (above) happens before any destination write; post-write cancellation is otherwise covered by the pre-existing download, clipboard-upload, folder-download and single-file tests.
- Not yet covered: storage exhaustion during execution after some items were published, and cancellation part-way through a remote-to-remote folder copy.
