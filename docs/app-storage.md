...
Values live in the desktop's local IndexedDB database, separate from host profiles, credentials and installed packages. They survive connection changes and app updates. All windows and versions in one installation lineage share their committed values. Removing a package retires that lineage: its local records may remain until storage cleanup, but a later package that reuses the same app ID receives a new principal and cannot access them. Package identity is not publisher authentication, so review updates as code that can access that installation's existing data.
...
Both namespaces expose `get`, `put`, `remove` and `list`. `get` returns `{ value, revision }` or `null`. Values are JSON, including JSON `null`; missing data is distinguished by the outer result. Each namespace is limited to 256 keys and 16 Mi UTF-16 units of serialized values. An `AbortSignal` is the optional final argument to each method.
...
`npm test -- src/extensions/app-storage.test.ts` checks host-owned identity binding, namespace separation, cumulative quota decisions, argument validation, permissions and channel-close cancellation. The desktop integration fixture exercises the public client through its opaque frame, plus real IndexedDB connections for concurrent writes, stale deletion, canceled writes and Unicode key pagination. It also remembers a note in one app version and restores it in an update from the same installation lineage before cleaning up the fixture data. These are synthetic local records, with no real remote writes.
