...
### Multiple selection and reviewed batch deletion

Files supports Ctrl/Cmd-click to toggle items, Shift-click and Shift-arrow/Home/End ranges in the displayed sort order, Ctrl/Cmd+A for all visible items, Ctrl/Cmd-arrow to move focus without changing selection, and Ctrl/Cmd+Space to toggle the focused row. Escape or a click on the empty listing clears selection. Right-clicking an already-selected row retains the group; right-clicking another row selects it alone. The toolbar actions button exposes the selection menu. Text fields retain native editing shortcuts.

Copy names/paths writes newline-separated text in displayed order. This is text copying, not remote file Copy/Paste. Selection belongs to each window, survives sorting and same-folder refresh, follows explicit provider relocation mappings, and drops entries hidden by filtering. Navigating to a different folder clears it. Rename, move, Cut/Paste, Copy to folder and download remain single-item actions; they never silently act on one member of a multi-selection.

Delete on a group reviews up to 100 items with exact names and locations and captures their listing revisions and original service binding. Confirmation deletes sequentially through the existing `removeEntry` service. The first failure stops the batch; results distinguish Deleted, Not confirmed and Not attempted. There is no automatic retry or rollback of completed deletions. Stop after current item allows the in-flight request to finish and skips the rest. Connection loss/replacement prevents later items from starting. Closing a running window is blocked by its busy state. Done closes the report and refreshes the listing; a retry requires a new selection/review. Nonempty folders still refuse deletion, and symlink targets remain untouched.

The `selection.html` browser fixture uses two actual Files components and a manually completed fake deletion service. Checks passed pointer/keyboard selection, exact multi-path copy, filtering, refresh retention, independent windows, review cancellation without writes, successful deletion, partial permission failure, stop-after-current, simulated connection loss and shared listing refresh. Unit tests cover selection ranges, opaque mappings, whole-batch validation, captured revisions and cancellation/failure boundaries. All 78 frontend tests and the standard Windows debug build passed. Native batch UI and physical-network-loss checks remain separate; this checkpoint performs no real remote deletion.
...
Only files, symlinks and **empty** directories can be deleted. There is no recursive delete, remote trash, undo or cross-host move. Regular-file Copy to folder is available through the transfer queue; directory copy and remote Copy/Paste remain pending. Already-open editors follow confirmed workspace rename/move results, including descendants of folders, while preserving drafts and conflict revisions; see [editor coordination](text-editor.md). [Files navigation, history and previews](file-navigation.md) also follow confirmed workspace relocations. Externally initiated moves are not tracked. Regular-file upload/download and cancellation are described in [transfers](transfers.md). Navigation is provider-owned; the production adapter still uses POSIX SFTP conventions. Other production providers and composite adapters remain backlog items.
