...
- A remote text editor with independent windows, undo/redo, find, word wrap, clipboard actions and saving existing UTF-8 files up to 256 KiB. Atomic SFTP replacement and revision checks detect conflicts; unsupported servers keep preview/draft access. [Save behavior and limits](docs/text-editor.md).
...
- The file explorer does not yet upload, download, create, rename, or delete files. Existing text files can be edited through the Editor. **The terminal is a real shell with all permissions of the authenticated account**, including root when selected.
- Editor drafts survive workspace switches and connection loss. Deliberate window/workspace/app closure guards unsaved work; forced termination can still lose in-memory drafts. Save As and crash recovery are not implemented yet.
