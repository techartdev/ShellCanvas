    ...
    async fn cancelled_root_preparation_closes_only_its_dedicated_sftp_channel() {
        ...
        for after_realpath in [false, true] {
            ...
            let preparation = tokio::spawn(async move {
                SftpMount::new(service, "/fixture", false).await
            });
            ...
            tokio::time::timeout(std::time::Duration::from_secs(2), peer).await.unwrap().unwrap();
            assert!(!other.channel_closed.load(std::sync::atomic::Ordering::Acquire),
